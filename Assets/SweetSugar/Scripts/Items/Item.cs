...
//        transform.position = destPos;// square.transform.position;
        ...
        private IEnumerator DestroyCor(bool showScore = false, bool particles = true, Item explodedItem = null, bool explEffect = false)
        {
            ...
            if(PlayerPrefs.GetInt("OpenLevel")>=1 && (PlayerPrefs.GetInt("OpenLevel") <= 30))
                {
                LevelManager.Score += 1;
                  }
            if (PlayerPrefs.GetInt("OpenLevel") >= 31 && (PlayerPrefs.GetInt("OpenLevel") <= 60))
                {
                LevelManager.Score += 2;
            }
            if (PlayerPrefs.GetInt("OpenLevel") >= 61 )
            {
