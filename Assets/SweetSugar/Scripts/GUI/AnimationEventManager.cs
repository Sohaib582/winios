...
//            LevelManager.THIS.gameStatus = GameState.Map;
        ...
        public void Play()
        {
           ...
           // YodoAdss.instance.ShowInterstitialAds();
            ...
            if (gameObject.name == "MenuPreGameOver")
            {
                if (InitScript.Gems >= 20)
                {
                    //sent scores and type to server for buying moves
                    PlayerPrefs.SetInt("server", 0);
                    PlayerPrefs.SetInt("server", -20);
                    PlayerPrefs.SetString("type", "");
                    PlayerPrefs.SetString("type", "bought_moves");
#if UNITY_ANDROID || UNITY_IOS
                    GoogleSignInDemo.Instance.postScoresLevels();
#endif


                    InitScript.Instance.SpendGems(20);
        ...
        {
            //OFF FOR TESTING
//            YodoAdss.instance.ShowRewardedVideoAds();
            SoundBase.Instance.PlayOneShot(SoundBase.Instance.click);
        ...
        public void BuyCoins(int coins)
        {
            ...
            //PlayerPrefs.SetInt("scorecount", PlayerPrefs.GetInt("scorecount") + coins);
        }
...
/*#if UNITY_PURCHASING && UNITY_INAPPS
            ...
            print("afffffffffffffffffffffffffffffff");
#endif*/
        ...
        public void BuyLife(GameObject button)
        {
            ...
            {
                //send scores and type to server
                PlayerPrefs.SetInt("server", 0);
                PlayerPrefs.SetInt("server", -20);
                PlayerPrefs.SetString("type", "");
                PlayerPrefs.SetString("type", "bought_life");
#if UNITY_ANDROID || UNITY_IOS
                GoogleSignInDemo.Instance.postScoresLevels();
#endif
...
#region boosts
        ...
        public void BuyBoost(BoostType boostType, int price, int count, Action callback)
        {
            ...
            if (InitScript.Gems >= price)
            {
                //send scores and type to server
                PlayerPrefs.SetInt("server", 0);
                PlayerPrefs.SetInt("server", -price);
                PlayerPrefs.SetString("type", "");
                PlayerPrefs.SetString("type", "bought_boost");
#if UNITY_ANDROID || UNITY_IOS
                GoogleSignInDemo.Instance.postScoresLevels();
#endif
